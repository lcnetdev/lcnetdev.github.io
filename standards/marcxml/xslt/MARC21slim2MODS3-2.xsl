...
-->
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="//marc:collection">
				<modsCollection xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://lcnetdev.github.io/mods/v3 http://lcnetdev.github.io/standards/mods/v3/mods-3-2.xsd">
			...
			<xsl:otherwise>
				<mods xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="3.2" xsi:schemaLocation="http://lcnetdev.github.io/mods/v3 http://lcnetdev.github.io/standards/mods/v3/mods-3-2.xsd">
	...
	<xsl:template name="marcRecord">
		<xsl:variable name="leader" select="marc:leader"></xsl:variable>
		<xsl:variable name="leader6" select="substring($leader,7,1)"></xsl:variable>
		<xsl:variable name="leader7" select="substring($leader,8,1)"></xsl:variable>
		<xsl:variable name="controlField008" select="marc:controlfield[@tag=008]"></xsl:variable>
			...
			<titleInfo>
				...
				<xsl:choose>
					<xsl:when test="@ind2>0">
						<nonSort>
							<xsl:value-of select="substring($titleChop,1,@ind2)"></xsl:value-of>
						</nonSort>
						<title>
							<xsl:value-of select="substring($titleChop,@ind2+1)"></xsl:value-of>
						</title>
					</xsl:when>
					<xsl:otherwise>
						<title>
							<xsl:value-of select="$titleChop"></xsl:value-of>
						</title>
					</xsl:otherwise>
				</xsl:choose>
		...
		<xsl:for-each select="marc:datafield[@tag=100]">
			<name type="personal">
				<xsl:call-template name="nameABCDQ"></xsl:call-template>
		...
		<xsl:for-each select="marc:datafield[@tag=110]">
			<name type="corporate">
				<xsl:call-template name="nameABCDN"></xsl:call-template>
				<role>
					<roleTerm authority="marcrelator" type="text">creator</roleTerm>
				</role>
				<xsl:call-template name="role"></xsl:call-template>
		...
		<xsl:for-each select="marc:datafield[@tag=111]">
			<name type="conference">
				<xsl:call-template name="nameACDEQ"></xsl:call-template>
				<role>
					<roleTerm authority="marcrelator" type="text">creator</roleTerm>
				</role>
				<xsl:call-template name="role"></xsl:call-template>
			...
			<xsl:if test="translate($MARCpublicationCode,'|','')">
				<place>
					<placeTerm>
						...
						<xsl:value-of select="$MARCpublicationCode"></xsl:value-of>
					</placeTerm>
				</place>
			</xsl:if>
			...
			<xsl:if test="$controlField008-6='e' or $controlField008-6='p' or $controlField008-6='r' or $controlField008-6='t' or $controlField008-6='s'">
				<xsl:if test="$controlField008-7-10 and ($controlField008-7-10 != $dataField260c)">
					<dateIssued encoding="marc">
						<xsl:value-of select="$controlField008-7-10"></xsl:value-of>
					</dateIssued>
				</xsl:if>
			</xsl:if>
			...
			<xsl:if test="$controlField008-6='c' or $controlField008-6='d' or $controlField008-6='i' or $controlField008-6='k' or $controlField008-6='m' or $controlField008-6='q' or $controlField008-6='u'">
				<xsl:if test="$controlField008-11-14">
					<dateIssued encoding="marc" point="end">
						<xsl:value-of select="$controlField008-11-14"></xsl:value-of>
					</dateIssued>
				</xsl:if>
			</xsl:if>
		...
		<xsl:if test="$controlField008-35-37">
			<language>
				<languageTerm authority="iso639-2b" type="code">
					<xsl:value-of select="substring($controlField008,36,3)"></xsl:value-of>
				</languageTerm>
			</language>
		</xsl:if>
		<xsl:for-each select="marc:datafield[@tag=041]">
			<xsl:for-each select="marc:subfield[@code='a' or @code='b' or @code='d' or @code='e' or @code='f' or @code='g' or @code='h']">
				<xsl:variable name="langCodes" select="."></xsl:variable>
				<xsl:choose>
		...
		<xsl:for-each select="marc:datafield[@tag=010]">
			<identifier type="lccn">
				<xsl:call-template name="isInvalid"></xsl:call-template>
				<xsl:value-of select="normalize-space(marc:subfield[@code='a'])"></xsl:value-of>
			</identifier>
		</xsl:for-each>
