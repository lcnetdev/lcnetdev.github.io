<xsl:stylesheet xmlns="http://lcnetdev.github.io/mods/v3"
	...
	<!-- 
		...
		MARC21slim2MODS3-4 (Revision 2.18) 20141219
		
		Revision 2.18 - Added a xsl:when to deal with '#' and ' ' in $leader19 and $controlField008-18 - ws 2014/12/19
	...
	<xsl:template match="marc:collection">
		<modsCollection xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
			xsi:schemaLocation="http://lcnetdev.github.io/mods/v3 http://lcnetdev.github.io/standards/mods/v3/mods-3-4.xsd">
	...
	<xsl:template match="marc:record">
		...
		<mods version="3.4">
			...
			<xsl:if test="/marc:record">
				...
				<xsl:attribute name="xsi:schemaLocation">http://lcnetdev.github.io/mods/v3
					http://lcnetdev.github.io/standards/mods/v3/mods-3-4.xsd</xsl:attribute>
			</xsl:if>
			...
			<xsl:apply-templates
				...
				mode="titleInfo"/>
			<xsl:apply-templates
				select="marc:datafield[@tag='740'][@ind2 !='2'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'740')]"
				mode="titleInfo"/>
			<xsl:apply-templates
		...
		-->
			...
			<recordInfo>
				...
				<recordOrigin>Converted from MARCXML to MODS version 3.4 using
					MARC21slim2MODS3-4_XSL2-0.xsl (Revision 2.18 2014/12/19)</recordOrigin>
	...
	<xsl:template
		match="marc:datafield[@tag='740'][@ind2 !='2'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'740')]"
		mode="titleInfo">
	...
	<xsl:template name="genre">
		...
		<xsl:apply-templates
			select="marc:datafield[@tag='047'][@ind2='7'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'047')]"
			mode="genre"/>
	...
	<xsl:template
		match="marc:datafield[@tag='047'][@ind2='7'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'047')]"
		mode="genre">
	...
	<xsl:template
		...
		mode="physDesc">
		<extent>
			<xsl:value-of select="local:subfieldSelect(.,'abce3fg')"/>
		</extent>
	</xsl:template>
	<!-- 351 note-->
		...
		<subject>
			...
			<xsl:for-each select="marc:subfield[@code='b' or @code='c']">
				<temporal>
					...
					<!-- NOTE: I'm not sure why c is replaced by '-' but that is how it is done in the old version so here it is -->
	...
	<!-- NOTE: the way these are written in original, each subfield would output an identifier, 
		this does one id for each datafield, not each subfield, will need to be tested -->
