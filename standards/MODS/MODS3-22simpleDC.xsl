...
Revision 1.2    2012-11-30 <tmee@loc.gov>
		Added date funtionality to handle dates with no start and/or end points
...
-->
	...
	<xsl:template match="mods:originInfo">
		...
		<xsl:if test="mods:dateCreated[@point!='start']">
			<dc:date>
				<xsl:value-of select="mods:dateCreated"/>
			</dc:date>
		</xsl:if>
		<xsl:if test="mods:dateCaptured[@point!='start']">
			<dc:date>
				<xsl:value-of select="mods:dateCaptured"/>
			</dc:date>
		</xsl:if>
		<xsl:if test="mods:dateOther[@point!='start']">
			<dc:date>
				<xsl:value-of select="mods:dateOther"/>
			</dc:date>
		</xsl:if>
		
		<xsl:for-each select="mods:dateIssued[@point='start']">
		...
		<xsl:for-each select="mods:dateCreated[@point='start']">
			<xsl:variable name="dateName" select="local-name()"/>
			<dc:date>
				<xsl:value-of select="."/>-<xsl:value-of
					select="../*[local-name()=$dateName][@point='end']"/>
		...
		<xsl:for-each select="mods:dateCaptured[@point='start']">
			<xsl:variable name="dateName" select="local-name()"/>
			<dc:date>
				<xsl:value-of select="."/>-<xsl:value-of
					select="../*[local-name()=$dateName][@point='end']"/>
